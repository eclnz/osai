...
ai_system :: proc(s: ^State, dt: f32) {
	for &ai, i in s.ai.dense {
		...
		case .Patrol:
			pos := ecs.get_or(&s.position, e, Vec2{})
			col := ecs.get_or(&s.collider, e, Collider{})
			grounded := ecs.get_or(&s.grounded, e, Grounded{})
...
movement_system :: proc(s: ^State, dt: f32) {
	for &intent, i in s.intent.dense {
		e := s.intent.owners[i]

		params := ecs.get(&s.movement, e)
		if params == nil {
			continue
		}
		...
		if intent.jump_requested && grounded.on_ground {
			...
			append(&s.events.sounds, Sound_Request{sound = .Jump, position = ecs.get_or(&s.position, e, Vec2{})})
		}
...
integration_system :: proc(s: ^State, dt: f32) {
	for &pos, i in s.position.dense {
		...
		if prev := ecs.get(&s.previous_position, e); prev != nil {
			prev^ = pos
		}
		if vel := ecs.get(&s.velocity, e); vel != nil {
			pos += vel^ * dt
		}
	}
}
